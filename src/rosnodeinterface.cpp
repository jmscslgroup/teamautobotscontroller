...
// C/C++ source code generated on : Thu Nov 30 11:50:41 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"teamautobotscontroller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(teamautobotscontroller_M, (NULL));
        teamautobotscontroller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(teamautobotscontroller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(teamautobotscontroller_M)
                == (NULL)) || rtmGetStopRequested(teamautobotscontroller_M));
    ...
    void NodeInterface::terminate(void)
    {
      if (mBaseRateThread.get()) {
        ...
        teamautobotscontroller_terminate();
        mNode.reset();
      }
    }
    ...
    {
      mRunModel = (rtmGetErrorStatus(teamautobotscontroller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();

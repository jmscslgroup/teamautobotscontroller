...
echo "=========================="
echo "Installing App teamautobotscontroller"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for teamautobotscontroller..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/teamautobotscontroller/installRosPackages.sh
...
rosrun robot_upstart install teamautobotscontroller/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable teamautobotscontroller
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
